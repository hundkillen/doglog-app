...
class AIPatternAnalyzer {
    ...
    struct DogInsights {
        ...
        let noteRecommendations: [String]
    }
    ...
    func analyzeDog(_ dog: Dog, timeRange: AnalysisTimeRange = .allTime) -> DogInsights {
        ...
        let noteTips = generateNoteRecommendations(activities: activities, dailyRatings: dailyRatings)
        ...
        return DogInsights(
            ...
            noteRecommendations: noteTips
        )
    }

    // MARK: - Notes-driven recommendations (local, heuristic)
    private func generateNoteRecommendations(activities: [Activity], dailyRatings: [DailyRating]) -> [String] {
        // Collect recent notes (last 30 days)
        let thirtyDaysAgo = Calendar.current.date(byAdding: .day, value: -30, to: Date()) ?? Date()
        let recentActivityNotes = activities
            .filter { $0.date >= thirtyDaysAgo }
            .compactMap { $0.notes?.lowercased() }
        let recentDailyNotes = dailyRatings
            .filter { $0.date >= thirtyDaysAgo }
            .compactMap { $0.notes?.lowercased() }
        let allNotes = recentActivityNotes + recentDailyNotes
        guard !allNotes.isEmpty else { return [] }

        var tips: [String] = []

        // Keyword buckets
        let stressWords = ["stress", "orol", "rädd", "ängsl", "nerv", "overstim", "over-stim", "over arous", "overarous", "trött", "fatigue"]
        let barkingWords = ["skäll", "bark", "skäller", "barking"]
        let pullingWords = ["dra i koppel", "drar i koppel", "pull", "koppel", "lead", "leash"]
        let focusWords = ["svårt att fok", "distrah", "kan inte koncentr", "distractions", "focus"]
        let successWords = ["gick bra", "framsteg", "success", "funkade", "lyckades"]

        func notesContain(_ words: [String]) -> Bool {
            allNotes.contains { n in words.contains(where: { n.contains($0) }) }
        }

        if notesContain(stressWords) {
            tips.append("insights.note_tip_reduce_stress".localized)
        }
        if notesContain(barkingWords) {
            tips.append("insights.note_tip_barking".localized)
        }
        if notesContain(pullingWords) {
            tips.append("insights.note_tip_leash".localized)
        }
        if notesContain(focusWords) {
            tips.append("insights.note_tip_focus".localized)
        }
        if notesContain(successWords) {
            tips.append("insights.note_tip_build_on_success".localized)
        }

        // Fallback general guidance if few tips
        if tips.count < 2 {
            tips.append(contentsOf: [
                "insights.note_tip_consistency".localized,
                "insights.note_tip_short_sessions".localized
            ])
        }

        // Deduplicate and limit
        let unique = Array(NSOrderedSet(array: tips)) as! [String]
    ...
    private func calculateMoodImprovement(_ ratings: [DailyRating]) -> Double {
        ...
        // firstScore is 0 when every early rating was "bad"; dividing by it
        // yields infinity and Int(infinity) crashes. Define improvement from
        // zero as +100%.
        guard firstScore != 0 else {
            return secondScore > 0 ? 100.0 : 0.0
        }
    ...
    private func calculateActivityTrend(_ activities: [Activity]) -> PatternTrend {
        ...
        // Same division-by-zero guard as calculateMoodImprovement.
        guard firstScore != 0 else {
            return secondScore > 0 ? .improving : .stable
        }
    ...
    private func generateBehaviorInsights(_ activities: [Activity], _ dailyRatings: [DailyRating], _ patterns: [ActivityPattern]) -> [BehaviorInsight] {
        ...
        if let mostFrequent = patterns.first {
            insights.append(BehaviorInsight(
                title: "insights.favorite_activity".localized,
                description: String(format: "insights.favorite_activity_desc".localized, ActivityCatalog.shared.displayName(forStoredType: mostFrequent.activityType), Int(mostFrequent.successRate * 100)),
                confidence: 0.8,
                category: .activity
            ))
        }
        ...
        if goodDays >= 5 {
            insights.append(BehaviorInsight(
                title: "insights.great_week".localized,
                description: String(format: "insights.great_week_desc".localized, goodDays),
                confidence: 0.9,
                category: .mood
            ))
        }
        ...
        if !successfulActivities.isEmpty {
            ...
            insights.append(BehaviorInsight(
                title: "insights.high_success_activity".localized,
                description: String(format: "insights.high_success_activity_desc".localized, ActivityCatalog.shared.displayName(forStoredType: topActivity.activityType)),
                confidence: 0.85,
                category: .behavior
            ))
        }
    ...
    private func generateRecommendations(_ mood: MoodTrend, _ patterns: [ActivityPattern], _ insights: [BehaviorInsight]) -> [Recommendation] {
        ...
        if mood.direction == .down {
            recommendations.append(Recommendation(
                title: "insights.boost_mood_activities".localized,
                description: "insights.boost_mood_activities_desc".localized,
                priority: .high,
                category: .routine,
                actionable: true
            ))
        ...
        if let lowSuccessActivity = patterns.first(where: { $0.successRate < 0.5 }) {
            recommendations.append(Recommendation(
                title: String(format: "insights.improve_activity".localized, ActivityCatalog.shared.displayName(forStoredType: lowSuccessActivity.activityType)),
                description: String(format: "insights.improve_activity_desc".localized, ActivityCatalog.shared.displayName(forStoredType: lowSuccessActivity.activityType)),
                priority: .medium,
                category: .training,
                actionable: true
            ))
        ...
        if exerciseActivities.isEmpty || exerciseActivities.allSatisfy({ $0.frequency < 3 }) {
            recommendations.append(Recommendation(
                title: "insights.increase_exercise".localized,
                description: "insights.increase_exercise_desc".localized,
                priority: .high,
                category: .exercise,
                actionable: true
            ))
